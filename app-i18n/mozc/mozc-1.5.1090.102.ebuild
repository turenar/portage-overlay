...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="+dict_ut +dict_ut_altcanna +dict_ut_zipcode dict_ut_hatena
dict_ut_nicodic dict_sawa emacs +ibus scim +qt4 renderer"
...
src_configure() {
	if use dict_ut; then
		...
		if use dict_ut_zipcode; then
			...
			"$(PYTHON)" gen_zip_code_seed.py --zip_code=KEN_ALL.CSV --jigyosyo= \
				JIGYOSYO.CSV >> ../../data/dictionary/dictionary09.txt
				ruby18 get-entries.rb KEN_ALL.CSV
			cd ..
		fi
		einfo "check major ut dictionaries..."
		ruby18 12-* altcanna/altcanna.txt 300
		ruby18 12-* jinmei/jinmei.txt 20
		ruby18 12-* ekimei/ekimei.txt 300
		ruby18 12-* chimei/chimei.txt 300

		cat altcanna/altcanna.txt.r jinmei/jinmei.txt.r ekimei/ekimei.txt.r \
			chimei/chimei.txt.r > ut-dic1.txt

		ruby18 44-* mozcdic_all.txt ut-dic1.txt
		ruby18 36-* ut-dic1.txt.yomihyouki
		cat ut-dic1.txt.yomihyouki.cost mozcdic_all.txt > mozcdic_all.txt.utmajor

		einfo "check minor ut dictionaries..."
		ruby18 12-* skk/skk.txt 300
		ruby18 12-* edict/edict.txt 300
		if use dict_ut_hatena; then
			ruby18 12-* hatena/hatena.txt 300
		fi
		if use dict_ut_nicodic; then
			ruby18 12-* niconico/niconico.txt 300
		fi
		...
		ruby18 42-* mozcdic_all.txt.utmajor ut-dic2.txt
		ruby18 40-* mozcdic_all.txt.utmajor ut-dic2.txt.yomi
		ruby18 36-* ut-dic2.txt.yomi.hyouki

		cat *.cost mozcdic_all.txt > ut-check-va.txt
		ruby18 60-* ut-check-va.txt
		ruby18 62-* ut-check-va.txt.va
	...
	if use dict_sawa; then
		echo "・・・、、、＠＠＠＋＋＋" > "${S}"/data/dictionary/suggestion_filter.txt
		cp "${FILESDIR}"/dictionary*.txt "${S}"/data/dictionary/
		sed -i \
			's|^\(.*\)\(\.\./data/dictionary/dictionary09\.txt\)\(.*\)$$|\1\2\3\n\1../debian/add_dict/mozc+dict.txt\3|' \
			${S}/dictionary/dictionary.gyp
		# (^^;
		/bin/echo -e "(笑)\tわらい しょう わら かっこわらい かっこわら" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "(爆)\tばく かっこばく" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "⊂ニニニニ( ＾ω＾)ニニニ⊃\tぶーん" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "⊂二二二（ ＾ω＾）二⊃\tぶーん" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "≡≡≡cっ・ω・)っ\tずさー" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "≡≡≡c⌒っﾟДﾟ)っ\tずさー" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "⊂(ﾟДﾟ⊂⌒｀つ≡≡≡\tずさー" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "(」・ω・)」うー！\tうー" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "(／・ω・)／にゃー！\tにゃー" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "(」・ω・)」うー！(／・ω・)／にゃー！\tうーにゃー" >> ${S}/data/emoticon/emoticon.tsv
	    /bin/echo -e "てへぺろ(・ω<)\tてへぺろ" >> ${S}/data/emoticon/emoticon.tsv
	    /bin/echo -e "(・ω<)\tてへぺろ" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "ヽ(・ω・)/ｽﾞｺｰ\tずこー" >> ${S}/data/emoticon/emoticon.tsv
		/bin/echo -e "ヽ(・ω・)/\tずこー" >> ${S}/data/emoticon/emoticon.tsv

		# symbol.tsv
	    /bin/echo -e "\n記号\t⏎\tかいぎょう えんたー\t\tOTHER" >> ${S}/data/symbol/symbol.tsv
	fi
