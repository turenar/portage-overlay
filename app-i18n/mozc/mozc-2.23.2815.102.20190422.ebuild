...
if [[ "${PV}" == "9999" ]]; then
	...
	MOZC_VER="$(ver_cut 1-4)"
	NEOLOGD_VER="$(ver_cut 5)"
fi
...
if [[ "${PV}" == "9999" ]]; then
	...
	SRC_URI="https://github.com/google/${PN}/archive/${MOZC_GIT_REVISION}.tar.gz -> ${MOZC_P}.tar.gz
...
POST_DATA_VER="20190527"
DICT_UT_URI="https://turenar.xyz/pub/mozcdic-neologd-ut-${NEOLOGD_VER}.1.tar.xz"
DICT_UT_POST_DATA_URI="https://turenar.xyz/pub/mozcdic-neologd-ut-ken_all-${POST_DATA_VER}.zip
	https://turenar.xyz/pub/mozcdic-neologd-ut-jigyosyo-${POST_DATA_VER}.zip"
SRC_URI="${SRC_URI}
	dict_neologd? ( ${DICT_UT_URI} ${DICT_UT_POST_DATA_URI} )"
...
S="${WORKDIR}/${MOZC_P}/src"
DICT_UT_S="${WORKDIR}/mozcdic-neologd-ut-${NEOLOGD_VER}.1"
...
src_unpack() {
	if [[ "${PV}" == "9999" ]]; then
		...
		unpack ${MOZC_P}.tar.gz
		mv mozc-${MOZC_GIT_REVISION} ${MOZC_P} || die

		unpack japanese-usage-dictionary-${JAPANESE_USAGE_DICTIONARY_DATE}.tar.gz
		cp -p japanese-usage-dictionary-${JAPANESE_USAGE_DICTIONARY_GIT_REVISION}/usage_dict.txt ${MOZC_P}/src/third_party/japanese_usage_dictionary || die
	fi

	if use dict_neologd; then
		unpack $(basename ${DICT_UT_URI})
		unpack mozcdic-neologd-ut-ken_all-${POST_DATA_VER}.zip
		unpack mozcdic-neologd-ut-jigyosyo-${POST_DATA_VER}.zip
		mv KEN_ALL.CSV JIGYOSYO.CSV "${DICT_UT_S}/chimei"
		cd "${S}"
	fi
}
...
src_prepare() {
	...
	if use dict_neologd; then
		cd "${DICT_UT_S}"
		eapply -p0 "${FILESDIR}/neologd-build-20190422.patch"
		cd "${S}"
	fi
