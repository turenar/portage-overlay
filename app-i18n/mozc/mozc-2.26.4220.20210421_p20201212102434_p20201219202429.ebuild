...
if [[ "${PV}" == "9999" ]]; then
	...
	SRC_URI="https://github.com/google/${PN}/archive/${MOZC_GIT_REVISION}.tar.gz -> ${PN}-${PV%%_p*}-${MOZC_DATE}.tar.gz
		...
		dict_ut? ( https://turenar.xyz/pub/mozcdic-ut-${DICT_UT_VERSION}.tar.bz2 )"
fi
...
IUSE="debug dict_ut emacs fcitx4 +gui ibus renderer test"
...
src_unpack() {
	if [[ "${PV}" == "9999" ]]; then
		...
		if use dict_ut; then
			unpack mozcdic-ut-${DICT_UT_VERSION}.tar.bz2
		fi
	fi
}
...
src_prepare() {
	...
	if use dict_ut; then
		cat "${WORKDIR}/mozcdic-ut-${DICT_UT_VERSION}/mozcdic-ut-${DICT_UT_VERSION}.txt" \
