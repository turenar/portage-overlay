...
inherit check-reqs cmake-utils eutils flag-o-matic \
	python-single-r1 toolchain-funcs pax-utils prefix
...
RDEPEND="${COMMON_DEPEND}
	clang? ( !<=${CATEGORY}/clang-${PV}-r99 )"
PDEPEND="clang? ( =${CATEGORY}/clang-${PV}-r100 )"
...
pkg_pretend() {
	...
	if [[ ${MERGE_TYPE} != binary ]]; then
		...
		if ! $(tc-getCXX) -std=c++11 -o "${T}"/test.cxx.obj "${T}"/test.cxx; then
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DCMAKE_INSTALL_PREFIX="${EPREFIX}${SYSROOT}"
	)
...
src_install() {
	docinto /usr/share/doc/${CTARGET}-${PF}
	...
	if use clang; then
		...
		for i in "${clang_tools[@]:1}"; do
			rm "${ED%/}/usr/${CTARGET}/bin/${i}" || die
			dosym "clang-${CLANG_VERSION}" "${SYSROOT}/bin/${i}-${CLANG_VERSION}"
			dosym "${i}-${CLANG_VERSION}" "${SYSROOT}/bin/${i}"
		done

		# now prepend ${CHOST} and let the multilib-build.eclass symlink it
		#if ! multilib_is_native_abi; then
		#	# non-native? let's replace it with a simple wrapper
		for i in "${D}/${SYSROOT}/bin"/*; do
			local longbin=${CTARGET}-${i##*/}
			dosym "${SYSROOT}/bin/${longbin}-${CLANG_VERSION}" "${SYSROOT}/bin/${longbin}"
		done
		...
		#		_EOF_
		#		newbin "${T}"/wrapper.tmp "${CTARGET}-${i}-${CLANG_VERSION}"
		#	done
		#fi
	fi

	# avoid file collisions with sys-devel/llvm
	# rm ${D}/usr/share/doc/llvm-${PV}/README.txt.bz2
